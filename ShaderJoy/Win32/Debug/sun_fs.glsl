...
void main()
{
	...
	freqs[0] = texture2D( texture_channel_1, vec2( 0.01, 0.25 ) ).x;
	freqs[1] = texture2D( texture_channel_1, vec2( 0.07, 0.25 ) ).x;
	freqs[2] = texture2D( texture_channel_1, vec2( 0.15, 0.25 ) ).x;
	freqs[3] = texture2D( texture_channel_1, vec2( 0.30, 0.25 ) ).x;
	...
	// float global_time = 100.0;
	
	float time		= global_time * 0.1;
	...
	if( dist < radius ){
		...
		vec3 texSample 	= texture2D( texture_channel_1, newUv ).rgb;
		float uOff		= ( texSample.g * brightness * 4.5 + time );
		vec2 starUV		= newUv + vec2( uOff, 0.0 );
		starSphere		= texture2D( texture_channel_1, starUV ).rgb;
	}
